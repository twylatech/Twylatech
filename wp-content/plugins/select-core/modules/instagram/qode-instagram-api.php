...
include PITCH_CORE_MODULES_PATH . '/instagram/qode-instagram-helper.php';

/**
 * Class PitchQodeInstagramApi
 */
class PitchQodeInstagramApi {
    /**
    ...
    private $apiURL;
    /**
     * @var PitchQodeInstagramApi instance of current class
     */
     ...
     * @return string
     *
     * @see PitchQodeInstagramApi::buildCurrentPageURI()
     */
     ...
     *
     * @see PitchQodeInstagramApi::fetchData()
     */
     ...
     * @return stdClass
     *
     * @see PitchQodeInstagramApi::getAccessToken()
     */
    private function fetchData($limit = '', $tag = '', $transient = array('use_transient' => false)) {
        ...
        if($this->useTransients($transient) && get_transient($transient['transient_name'])) {
            ...
            if(is_wp_error($httpResponse) || (isset($httpBody['meta']['code']) && $httpBody['meta']['code'] !== 200)) {
                ...
                $returnObject->message = esc_html__( 'Can\'t connect with Instagram', 'select-core' );
        ...
        $returnObject->message = esc_html__( 'Success', 'select-core' );
    ...
    public function getAccessToken() {
        ...
        if(empty( $this->code)) {
            $returnObject->status = 'error';
            $returnObject->message = esc_html__( 'User has\'nt connected with Instagram', 'select-core' );
            return $returnObject;
        } else {
            $getTokenArgs = array(
                'body' => array(
            ...
            if(is_wp_error($httpResponse) || (isset($httpResponse['response']['code']) && $httpResponse['response']['code'] !== 200)) {
                $returnObject->status = 'error';
                $returnObject->message = esc_html__( 'Can\t connect with Instagram API', 'select-core' );
                return $returnObject;
            }
            ...
            if(isset($responseBody['access_token']) && isset($responseBody['user']['id'])) {
                ...
                $returnObject->status = 'error';
                $returnObject->message = esc_html__( 'Can\t get Instagram access token', 'select-core' );
                return $returnObject;
            }
        }
        ...
        //prepare return object and return it
        $returnObject->status = 'ok';
        $returnObject->message = esc_html__( 'Stored access token', 'select-core' );
    ...
    public function disconnectUser() {
       if($this->hasUserConnected()) {
           ...
           if ($response) {
               ...
               $responseObj->message = esc_html__('Couldn\'t disconnect from Instagram', 'select-core');
           }
