...
if( ! class_exists( 'PitchQodeSidebar' ) ){
	
	class PitchQodeSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','pitch');
		...
		function load_assets(){
			...
			wp_enqueue_script('pitch-sidebar' , PITCH_ROOT . '/framework/admin/assets/js/qodef-sidebar.js');
			wp_enqueue_style( 'pitch-sidebar' , PITCH_ROOT . '/framework/admin/assets/css/qodef-sidebar.css');
		}
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_attr__('Enter Name of the new Widget Area', 'pitch')."' name='qode-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_attr__('Add Widget Area', 'pitch')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('pitch_qode_filter_custom_widget_args', $args);
