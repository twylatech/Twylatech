...
add_filter( 'wp_setup_nav_menu_item', 'pitch_qode_add_custom_nav_fields');

// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'pitch_qode_update_custom_nav_fields', 10, 3 );

// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'pitch_qode_edit_walker', 10, 2 );


function pitch_qode_add_custom_nav_fields( $menu_item ) {
 ...
 * @return      void
*/
function pitch_qode_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
 ...
 * @return      void
*/
function pitch_qode_edit_walker($walker,$menu_id) {

	return 'PitchQodeWalkerNavMenuEditCustom';
...
if (!class_exists('PitchQodeWalkerMenuType1')) {
	class PitchQodeWalkerMenuType1 extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$wide_menu_background = pitch_qode_options()->getOptionValue( 'enable_wide_manu_background' ) == 'yes';
			
			$indent = str_repeat("\t", $depth);
			if($depth == 0){
				$out_div = '<div class="second'.$background_image_class.'" '.($wide_menu_background ? pitch_qode_get_inline_style($menu_image_background) : '').'><div class="inner">';
			}else{
				$out_div = '';
			}

			// build html
			$output .= "\n" . $indent . $out_div  .'<ul '.(!$wide_menu_background ? pitch_qode_get_inline_style($menu_image_background) : '').'>' . "\n";
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$wide_background = '';
			if(pitch_qode_options()->getOptionValue( 'enable_wide_manu_background' ) == "yes" ){
			...
			if($item->hide == ""){
                ...
                if($depth == 0 && pitch_qode_options()->getOptionValue( 'enable_dropdown_menu_item_icon' ) == "yes" ){
				...
				if(pitch_qode_options()->getOptionValue( 'menu_item_hover_animation' ) == "roll" && $depth == 0){
...
if (!class_exists('PitchQodeWalkerMenuType2')) {
	class PitchQodeWalkerMenuType2 extends Walker_Nav_Menu {
...
if (!class_exists('PitchQodeWalkerMenuType3')) {
    class PitchQodeWalkerMenuType3 extends Walker_Nav_Menu {
...
if (!class_exists('PitchQodeWalkerMenuType4')) {
    class PitchQodeWalkerMenuType4 extends Walker_Nav_Menu {
