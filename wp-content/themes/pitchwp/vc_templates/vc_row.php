...
$args = array_merge($args, pitch_qode_icon_collections()->getShortcodeParams('content_menu_'));
...
if ($icon_pack !== '' && $in_content_menu !== '') {
    ...
    $item_collection_icon_obj = pitch_qode_icon_collections()->getIconCollection($icon_pack);
...
if($row_type == 'row') {
	...
	if($video == "show_video"){
        ...
        if($use_row_as_full_screen_section == "yes"){
            ...
            $output .= '></div>';
            $output .= '<video class="full_screen_sections_video" width="1920" height="800" poster="'.esc_url($v_image).'" controls="controls" preload="auto" loop autoplay muted>';
            ...
            $output .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.get_template_directory_uri().'/js/flashmediaelement.swf">
										...
										<img src="'.esc_url($v_image).'" width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'pitch' ) . '"  alt="' . esc_attr__( 'Video thumb', 'pitch' ) . '"  />
								</object>
						</video>';
        } else {
	...
	if($logo_in_content_menu == 'logo_in_content_menu'){
		$output .= '<div class="logo">';
			...
			if( pitch_qode_options()->getOptionValue( 'content_menu_logo_image' ) != ""){
				$logo_image = pitch_qode_options()->getOptionValue( 'content_menu_logo_image' );
			}else if(pitch_qode_options()->getOptionValue( 'logo_image' ) != ""){
				$logo_image = pitch_qode_options()->getOptionValue( 'logo_image' );
			}
			$output .= '<a href="'.esc_url(home_url('/')).'"><img src="'.esc_url($logo_image).'" alt="' . esc_attr__( 'Logo', 'pitch' ) . '" /></a>';
		$output .= '</div>';
	}
	...
	if($custom_widget_area !== ''){
		$output .= '<div class="custom_widget_area"><div class="custom_widget_area_inner"><div class="custom_widget_area_inner2">';
			$output .= pitch_qode_get_dynamic_sidebar($custom_widget_area);
		$output .= '</div></div></div>';
	}
